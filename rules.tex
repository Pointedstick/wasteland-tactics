...
When a Vehicle/Monster model makes a Fight action that would bring it into contact with a non-Vehicle/Monster unit that it is not currently in contact with, then after it moves, each target unit must pass a Will roll or else take a Withdraw Reaction away from the Vehicle/Monster model. 
...
Before rolling to hit, the targeted unit may make a Reaction, if any are applicable.
...
Each targeted enemy unit which has one or fewer Suppression Markers and is not currently in base contact with any of your models may then make a Reaction.
...
At various times, a unit may be eligible or required to make a Reaction, as mentioned in other parts of the rules.

Reactions are always made outside of the normal sequence of activation and regardless of whether the reacting unit had already activated or how many actions it has available. After making a Reaction, the reacting unit always gains a suppression marker.

Here are the  different reactions that a unit can make:



\section*{Withdraw}
The unit immediately makes a Move action away from whatever enemy unit provoked the Reaction. All models in the unit must move the full distance in a straight line, with no maneuvering or routing around obstacles permitted.

If any of the unit's models cannot complete the movement because they would come into base contact with impassible terrain or any enemy models, the unit is destroyed; morale collapses after finding the retreat corridor blocked or filled with enemy forces!



\section*{Dig In}
The unit immediately gains the benefits of the Defend action.



\section*{Counter-Attack}
This Reaction may only be done if the distance between the two units 18`` away or closer. The reacting unit makes a Will roll. If the roll is passed, it immediately takes a free Shoot action against the unit that provoked the reaction. If it is reacting to a Fight action, all of its ranged weapons are automatically considered to be in range during this free Shoot action.
