...
\subsubsection*{Sniper:} This weapon may be used to fire at units that are not the closest visible target without requiring a Will roll, and you may choose which models to allocate Life rolls to. However the weapon's Damage value is reduced to 0 against Vehicles without the Open rule.
...
\textit{For example: rocky ground, ruins and other dense rubble-strewn areas, sandbag walls, forests, thick vegetation, streams and shallow rivers, cratered ground, swamps}
...
For ``Vehicle/Monster'' units, you must instead choose whether to apply a penalty of -6 Move, or else count the area as Dangerous Terrain 9.
...
\subsubsection*{5 Dark/Foggy:} Units that opt to move faster than 6'' during a Move or Fight action count as being inside Dangerous Terrain 9+, and every unit suffers a -1 Shoot penalty for each full 10'' separating the firing unit and its target.

\subsubsection*{6 Muddy/Swampy:} All areas of Open Terrain become Difficult Terrain. Areas that were already Difficult terrain reduce movement by 3.
