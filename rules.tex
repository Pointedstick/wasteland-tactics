...
During the game, models will be called upon to ``make a Will roll,'' ``make a Life roll,'' and so on. To do this, roll a ten-sided die (also known as a D10) and compare the number to the characteristic in question. For example, a model with a Will characteristic of 6+ will pass Will rolls on a roll of 6 or higher.

If your D10s show the number 0 instead of 10, treat 0 as 10.

For Shoot and Fight rolls, the die roll need to succeed is listed on the relevant weapon itself.
...
\begin{itemize}
    ...
    \item \textbf{[number]+:} The weapon’s chance of hitting its target. When making a shoot or Fight action with this weapon, you must roll equal to or above this number to score a hit with each attack. ``Autohit'' means that no rolls are necessary; all of the weapon’s attacks automatically hit!
...
\subsubsection*{Critical:} This weapon is capable of dealing terrible damage with a lucky hit. If it would automatically hit, roll anyway. For each die that shows a 10, the target fails a life roll automatically, and double the value of the weapon's Deadly rule, if it has one. If not, it becomes Deadly 2.
...
\subsubsection*{Unstable:} This weapon may explode! If it would automatically hit, roll anyway. For each die that shows a 1, the firing model fails a Life roll.
...
When a unit takes a Shoot action targeting an enemy unit with more than 3 models whose ``footprint'' on the tabletop is at least twice as deep as it is wide from the perspective of the firing unit, it gains a +1 shoot bonus and the enemy unit receives a -1 Life penalty.

Similarly, when a unit with more than 3 models takes a Shoot action, if it has a tabletop footprint that it at least twice as deep as it is wide from the perspective of the target unit, it gains a +1 Shoot bonus.

If a unit's footprint is at all ambiguous, err on the side of applying no bonuses or penalties.
