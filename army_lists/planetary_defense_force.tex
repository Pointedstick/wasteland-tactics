% Inherit standard army list-specific formatting
\input{../globals/globals_army_list.tex}






\begin{document}

\title{Planetary Defense Force}
\author{Nate Graham}
\date{\today\\ Version 0.0.5}

\maketitle

The Planetary Defense Force is an endless army grinding relentlessly forward with numberless masses of men, guns and tanks. An individual soldier is weak and unthreatening, but alongside dozens or hundreds of his comrades organized into complex formations, and backed up by the might of heavy weapons and armored vehicles, the combined whole is a force to be reckoned with.

You might enjoy playing as the Planetary Defense Force if you appreciate military history or tactics and empathize with the common soldier on the front lines of a conflict much larger than him.






% Common weapon definitions
% =========================

% Melee weapons
% -------------
% Handheld
\newcommand{\CombatKnife}[1][6+]{Combat Knife: R0 A1 {#1} D0}
\newcommand{\DemoCharges}[1][6+]{Demo Charges: R0 A1 {#1} D6 Critical 9+, Deadly D5+2, Slow, Unstable 1, May only be used against Vehicle or Monster models}
\newcommand{\EnergySword}[1][5+]{Energy Sword: R0 A2 {#1} D6}
\newcommand{\HeavyMeleeWeapon}[1][6+]{Heavy Melee Weapon: R0 A2 {#1} D1}
\newcommand{\HugeMeleeWeapon}[1][4+]{Huge Melee Weapon: R0 A2 {#1} D3 [Deadly 2]}

% Vehicle-borne
\newcommand{\BladesAndSaws}[1][7+]{Blades and Saws: R0 A3 {#1} D3 [Deadly 2]}
\newcommand{\CrushingTracks}[1][9+]{Crushing Tracks: R0 A3 {#1} D8 [Deadly 4]}
\newcommand{\ReinforcedRam}[1][6+]{Reinforced Ram: R0 A1 {#1} D5 [Deadly 4]}


% Ranged weapons
% --------------
% Handheld
\newcommand{\AntimatterGun}[1][6+]{Antimatter Gun: R20 A1 {#1} D6, [Deadly 2, Unstable]}
\newcommand{\AutocannonCrewed}[1][6+]{Autocannon: R30 A2 {#1} D4 [Deadly 2, Crew-Served, Heavy, Slow]}
\newcommand{\BattlePistol}[1][5+]{Battle Pistol: R10 A1 {#1} D2 [Assault]}
\newcommand{\BattleRifle}[1][5+]{Battle Rifle: R20 A1 {#1} D2}
\newcommand{\Carbine}[1][7+]{Carbine: R15 A2 {#1} D0 [Assault]}
\newcommand{\Flamethrower}{Flamethrower: R8 A4 Autohit D1 [Assault, Slow, Spray, Terrifying 1]}
\newcommand{\GrenadeLauncher}[1][6+]{Grenade Launcher: R20 A3 {#1} D1 [Area Effect, Assault, Slow, Suppressive 1]}
\newcommand{\Grenades}[1][5+]{Grenades: R6 A3 {#1} D1 [Area Effect, Assault, Suppressive 0]}
\newcommand{\HeavyMachineGunCrewed}[1][6+]{Heavy Machine Gun: R20 A3 {#1} D2 [Crew-Served, Heavy, Suppressive 1]}
\newcommand{\HugeCombatShotgun}[1][7+]{Huge Combat Shotgun: R10 A2 {#1} D2 [Assault]}
\newcommand{\MagmaGun}[1][6+]{Magma Gun: R10 A1 {#1} D8 [Critical 8+, Deadly 2D5, Slow]}
\newcommand{\MortarCrewed}[1][6+]{Mortar: R30 A6 {#1} D1 [Area Effect, Crew-Served, Heavy, Slow, Suppressive 0]}
\newcommand{\Rifle}[1][6+]{Rifle: R20 A1 {#1} D0}
\newcommand{\RocketLauncherCrewed}[1][6+]{Rocket Launcher: R40 A1 {#1} D6 [Crew-Served, Critical 9+, Deadly D10, Heavy, Slow]}
\newcommand{\SniperRifle}[1][5+]{Sniper Rifle: R30 A1 {#1} D6 [Crew-Served, Critical 8+, Deadly 2, Heavy, Sniper, Terrifying 1]}

% Vehicle-borne
\newcommand{\AntiTankCannon}[1][5+]{Anti-Tank Cannon: R40 A1 {#1} D8 [Critical 9+, Deadly D10+1, Heavy, Slow]}
\newcommand{\ArtilleryCannon}[1][5+]{Artillery Cannon: R80 A8 {#1} [Heavy, Indirect, Slow, Suppressive 1]}
\newcommand{\Autocannon}[1][5+]{Autocannon: R30 A2 {#1} D4 [Deadly 2, Heavy, Slow]}
\newcommand{\GatlingCannon}[1][5+]{Gatling Cannon: R30 A7 {#1} D2 [Heavy, Suppressive 3]}
\newcommand{\HeavyFlamethrower}{Heavy Flamethrower: R8 A6 Autohit D2 [Assault, Slow, Spray, Terrifying 2]}
\newcommand{\HeavyMachineGun}[1][5+]{Heavy Machine Gun: R20 A3 {#1} D2 [Heavy, Suppressive 1]}
\newcommand{\HeavyMachineGuns}[1][5+]{Heavy Machine Guns: R20 A6 {#1} D2 [Heavy, Suppressive 1]}
\newcommand{\GiantFlamethrower}{Giant Flamethrower: R10 A12 Autohit D2 [Assault, Slow, Spray, Terrifying 2]}
\newcommand{\LongRangeCannon}[1][5+]{Long-Range Cannon: R40 A6 {#1} D5 [Area Effect, Deadly 3, Heavy, Slow]}
\newcommand{\RocketLauncher}[1][5+]{Rocket Launcher: R40 A1 {#1} D6 [Critical 9+, Deadly D10, Heavy, Slow]}
\newcommand{\SiegeCannon}[1][5+]{Siege Cannon R20 A8 {#1} D8 [Area Effect, Deadly D5+3, Heavy, Slow, Terrifying 1]}






\section*{Army-specific rules}

\subsection*{New unit rules}

\subsubsection*{Endless Army:} You may take up double the number of Troops units permitted by the game type.

\subsection*{New weapon rules}

\subsubsection*{Crew-Served:} This weapon requires an additional model serving as a loader to operate at full capability. Each weapon in the unit requires a different loader model, and the loader may not shoot its own weapons during a shoot action when the weapon is fired. If no suitable loader is present, the weapon gains the Slow rule. If it already had the Slow rule, it loses 1 from its A value. If the weapon's A value is already 1, then it may not be fired at all.

\subsection*{Special commands}

\subsubsection*{Call in fire support!} Target a visible enemy unit and roll a die. On a 4+, that enemy unit is attacked, but on a roll of 1, the owner of the target unit chooses one of your units to be attacked instead! The affected unit is attacked by a weapon with the following characteristics: R1000 A6 8+ D4 [Area Effect, Deadly 2, Suppressive 2]

\subsubsection*{Finish them off!} The unit gains +2 Shoot when targeting an enemy unit that has any suppression markers.

\subsubsection*{Make for the ultimate sacrifice!} The unit gains the “Bodyguards 4+” special rule for the duration of the round.



\pagebreak



\begin{small}

\section*{Command}
\input{army_list_table.tex}


% Unit
Command Squad (5)
&
% Cost
40
&
% Move
6
&
% Will
4+
&
% Life
6+
&
% Weapons
\Rifle[5+]

\CombatKnife[5+]

One model also has an \EnergySword
&
% Special rules
Leadership 2, Medic 8+



\\



% Unit
Drill Sergeant 
&
% Cost
30
&
% Move
6
&
% Will
3+
&
% Life
6+
&
% Weapons
\BattlePistol

\EnergySword
&
% Special rules
Leadership 1, Price of Failure 1

\end{tabular}








\section*{Spearhead}
\input{army_list_table.tex}


% Unit
Heavy Trooper Squad (5-10)
&
% Cost
9 each
&
% Move
5
&
% Will
5+
&
% Life
5+
&
% Weapons
\BattleRifle

\CombatKnife

\Grenades
&
% Special rules
Acute Senses, Medic 8+



\\



% Unit
Special Weapons Squad (5-10)
&
% Cost
12 each
&
% Move
6
&
% Will
6+
&
% Life
6+
&
% Weapons
\Rifle

\CombatKnife

\DemoCharges

Half of the models (rounded down) also have one of the following (you may mix and match):
\begin{itemize}
    \item \Flamethrower
    \item \GrenadeLauncher
    \item \MagmaGun
    \item \AntimatterGun
\end{itemize}
&
% Special rules
Medic 8+



\\



% Unit
Paratrooper Squad (10)
&
% Cost
90
&
% Move
7
&
% Will
6+
&
% Life
6+
&
% Weapons
\Carbine

\CombatKnife

\Grenades
&
% Special rules
Ambush, Scouts 1, Medic 8+



\\



% Unit
Sniper Team (5)
&
% Cost
30
&
% Move
7
&
% Will
5+
&
% Life
6+
&
% Weapons
2 models have a \SniperRifle

3 models have a \Carbine

\CombatKnife
&
% Special rules
Acute Senses, Scouts 1, Stealth 2, Strider (horizontal difficult terrain and vertically impassible terrain)


\\


% Unit
Impressed Psychic
&
% Cost
15
&
% Move
6
&
% Will
5+
&
% Life
8+
&
% Weapons
\CombatKnife
&
% Special rules
Caster 1


\\


% Unit
Ogres (3-6)
&
% Cost
20 each
&
% Move
6
&
% Will
7+
&
% Life
5+
&
% Weapons
\HugeMeleeWeapon

\HugeCombatShotgun
&
% Special rules
Bloodthirsty 1, Tough 3
\end{tabular}








\section*{Troops}
\input{army_list_table.tex}


% Unit
Infantry Squad (10)
&
% Cost
70
&
% Move
6
&
% Will
6+
&
% Life
6+
&
% Weapons
\Rifle

\CombatKnife

One model also has a \HeavyMachineGunCrewed

A different model also has one of the following:
\begin{itemize}
    \item \Flamethrower
    \item \GrenadeLauncher
\end{itemize}
&
% Special rules
Medic 9+



\\


% Unit
Conscript Mob (20)
&
% Cost
100
&
% Move
6
&
% Will
7+
&
% Life
7+
&
% Weapons
\Rifle[7+]

\CombatKnife[7+]
&
% Special rules
Bodyguards 6+
\end{tabular}







\section*{Fast}
\input{army_list_table.tex}


% Unit
Heavy Cavalry (3-10)
&
% Cost
15 each
&
% Move
10
&
% Will
6+
&
% Life
5+
&
% Weapons
\HeavyMeleeWeapon

\Carbine

\Grenades
&
% Special rules
Scouts 1, Reach, Tough 2



\\



% Unit
Dragon Tank
&
% Cost
120
&
% Move
10
&
% Will
6+
&
% Life
1+
&
% Weapons
\CrushingTracks

\HeavyMachineGun

\GiantFlamethrower
&
% Special rules
Hardened, Tough 8, Vehicle 6+, Vulnerable 1



\\



% Unit
Scout Walkers (1-3)
&
% Cost
40 each
&
% Move
10
&
% Will
6+
&
% Life
3+
&
% Weapons
\BladesAndSaws

Each model also has one of the following (you may mix and match):
\begin{itemize}
    \item \HeavyFlamethrower
    \item \HeavyMachineGun
    \item \Autocannon
    \item \RocketLauncher
\end{itemize}
&
% Special rules
Open, Scouts 1, Tough 5, Vehicle 9+
\end{tabular}








\section*{Transports}
\input{army_list_table.tex}


% Unit
Light APC
&
% Cost
40
&
% Move
12
&
% Will
6+
&
% Life
3+
&
% Weapons
\ReinforcedRam

\HeavyMachineGun
&
% Special rules
Open, Tough 6, Transport 7, Vehicle 7+, Vulnerable 1



\\



% Unit
Heavy APC
&
% Cost
60
&
% Move
10      
&
% Will
6+
&
% Life
1+
&
% Weapons
\ReinforcedRam

Pick one:
\begin{itemize}
    \item \HeavyFlamethrower
    \item \HeavyMachineGun
    \item \Autocannon
    \item \RocketLauncher
\end{itemize}
&
% Special rules
Hardened, Tough 8, Transport 12, Vehicle 8+, Vulnerable 1
\end{tabular}







\section*{Heavy}
\input{army_list_table.tex}


% Unit
Battle Tank
&
% Cost
200
&
% Move
10
&
% Will
6+
&
% Life
0+
&
% Weapons
\CrushingTracks

\HeavyMachineGuns

Pick one:
\begin{itemize}
    \item \AntiTankCannon
    \item \GatlingCannon
    \item \LongRangeCannon
    \item \SiegeCannon
\end{itemize}
&
% Special rules
Hardened, Tough 10, Vehicle 8+, Vulnerable 1




\\




% Unit
Mechanized Artillery
&
% Cost
120
&
% Move
10
&
% Will
6+
&
% Life
1+
&
% Weapons
\CrushingTracks

\HeavyMachineGun

\ArtilleryCannon
&
% Special rules
Hardened, Tough 8, Vehicle 8+, Vulnerable 1



\\



% Unit
Heavy Weapons Squad (6-10)
&
% Cost
10 each
&
% Move
6
&
% Will
6+
&
% Life
6+
&
% Weapons
\Rifle

\CombatKnife

Half of the models (rounded down) also have one of the following (you may mix and match):
\begin{itemize}
    \item \AutocannonCrewed
    \item \HeavyMachineGunCrewed
    \item \MortarCrewed
    \item \RocketLauncherCrewed
\end{itemize}
&
% Special rules

\end{tabular}

\end{small}


\end{document}
